...
def fig_int_int2 (df, year, interv=interv):
    ...
    fig3 = go.Figure()
    fig4 = go.Figure()
    stats = pd.DataFrame([], columns=['annee', 'ampli', 'categorie', 'nbtotal', 'moyenne Gycm2', 'ecartType', 'median Gycm2', '75%centile'])
    stats_list=[]    
    ...
    for ampli in ampli_list:
        ...
        # for cat in cat_list:      
            matint=[]
            matint=mat['DATEINTERV'][mat['AMPLI']==ampli]
            ...
            stats_list.append([year, ampli, interv, len(yy), round(yy.mean(),3), round(yy.std(),3), round(yy.median(),3), round(np.percentile(yy, 75),3)])
    ...
    stats_list.append([year, 'TOTAL', interv, len(ytot), round(ytot.mean(),3), round(ytot.std(),3), round(ytot.median(),3), round(np.percentile(ytot, 75),3)])
