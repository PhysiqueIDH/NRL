...
def fig_int_GENERAL_1param(df, year, param2, interv=None):
    ...
    stats = pd.DataFrame([], columns=['Annee', switch(param2), 'Nbtotal', 'PKS_moyen(Gycm2)',
                                      'EcartType(Gycm2)', 'Median(Gycm2)', 'Temps_moyen(s)', 'EcartType(s)',
                                      'Median(s)'])
    ...
    for leg in legend_list:
        ...
        stats_list.append(
            [year, leg, len(yy), round(yy.mean(), 3), round(yy.std(), 3), round(yy.median(), 3),
             round(tt.mean(), 3), round(tt.std(), 3), round(tt.median(), 3)])
        # stats_list1.append(
        #     [year, leg, len(yy), round(yy.mean(), 3), round(yy.std(), 3), round(yy.median(), 3),
        #      round(tt.mean(), 3), round(tt.std(), 3), round(tt.median(), 3)])
    ...
    fig2.write_html(
        sys.path[0] + "\\results\\" + switch(param2) + "\\" + str(yearmin) + "TEMPS.html")
    fig3.write_html(
